...
@IonicPage()
@Component({
  selector: 'page-cateringlist',
  templateUrl: 'cateringlist.html',
})
export class CateringlistPage {
...
location_id: any;
  ...
  constructor(public apis: ApisProvider,public loadingCtrl: LoadingController,public alertCtrl: AlertController,public navCtrl: NavController, public navParams: NavParams) {
    ...
    this.apis.fetch_caterers(this.location_id).then((result) => {
      this.body = result;    
  ...
  openMenu(item){
    ...
    this.navCtrl.push("CateringMenuPage", {restaurant_id: item.id, user_details: this.user_details });
  }
